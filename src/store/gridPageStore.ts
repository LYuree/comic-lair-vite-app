...
export class GridPageStore {
  ...
  setSearchFormValue = (textInput: string) => {
    this.searchFormValue = textInput;
  };
  ...
  toggleCategoryCheckbox = (id: string, checkedValue: boolean) => {
    this.setCategoryCheckboxes(
      this.categoryCheckboxes.map((checkbox) =>
        checkbox.id === id ? { ...checkbox, checked: checkedValue } : checkbox
      )
    );
  };

  setCoverCheckboxes = (
    checkboxes: { id: string; coverType: string; checked: boolean }[]
  ) => {
    this.coverCheckboxes = checkboxes;
  };

  toggleCoverCheckbox = (id: string, checked: boolean) => {
    const index = this.coverCheckboxes.findIndex(
  ...
  setBrandCheckboxes = (
    checkboxes: { id: string; brandName: string; checked: boolean }[]
  ) => {
    this.brandCheckboxes = checkboxes;
  };

  toggleBrandCheckbox = (id: string, checked: boolean) => {
    const index = this.brandCheckboxes.findIndex(
      (checkbox) => checkbox.id === id
    );
    if (index !== -1) {
