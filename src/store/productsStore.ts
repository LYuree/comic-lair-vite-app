...
import {
  ...
  IProductItem,
  ProductsData,
} from "../api/products/fetchProducts.ts";
    ...
    switch (sortingMethod) {
      ...
      case "cheapest_first":
        sortedProducts.data.sort(
          (a: IProductItem, b: IProductItem) => a["price"] - b["price"]
        );
        break;
      case "expensive_first":
        sortedProducts.data.sort(
          (a: IProductItem, b: IProductItem) => b["price"] - a["price"]
        );
        break;
      case "A_Z":
        sortedProducts.data.sort((a: IProductItem, b: IProductItem) =>
          a["name"].localeCompare(b["name"])
        );
        break;
      case "Z_A":
        sortedProducts.data
          .sort((a: IProductItem, b: IProductItem) =>
            a["name"].localeCompare(b["name"])
          )
          .reverse();
        break;
      ...
      default:
        this.setError(
          "Ошибка в типе сортировки (выполнен вход в ветку switch)."
        );
        break;
    }
