...
const CheckOutPage = observer(() => {
    const {cartStore: {cartProducts, fetchCartProducts, totalCost}} = rootStore;
    useEffect(() => {
        fetchCartProducts();
    }, []);
    ...
    return ( 
        <div className="flex py-8 px-16">
            ...
            <div className="pt-16 px-8 mt-8">
                <span className="text-start font-bold border-y border-black py-8">СУММА ЗАКАЗА: <span className="text-[green]">{totalCost}</span></span>
            </div>
        </div>
     );
});
